...
export function RecipesPage() {
  const location = useLocation(); const navigate = useNavigate(); const [recipes, setRecipes] = useState([]); const [loading, setLoading] = useState(true); const [error, setError] = useState(''); const [formRecipe, setFormRecipe] = useState(null); const [formLoading, setFormLoading] = useState(false); const [formError, setFormError] = useState('')
  ...
  useEffect(() => { if (!location.state?.openRecipeForm) return; openForm(); navigate('/recetas', { replace: true, state: null }) }, [location.state, navigate])
